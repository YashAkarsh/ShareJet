...
@app.post("/upload")
def upload_files(files: list[UploadFile] = File(...), device_name: str = "YashPhone"):
    ...
    # Prepare final zip path
    zip_filename = f"pending.zip"
    zip_path = os.path.join(PENDING_FOLDER,device_name,zip_filename)

    # Save the zip buffer to the pending folder
    with open(zip_path, "wb") as f:
        f.write(zip_buffer.getvalue())

    return {"Message": f"All files zipped and stored in pending_files as {zip_filename}"}
